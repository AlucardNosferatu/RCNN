    ...
    for e, annotation_file in enumerate(os.listdir(annotation_path)):
        ...
        for e_roi, result in enumerate(ss_results):
            if flag == 0:
                ...
                if (len(train_labels) - train_labels.count(0)) < file_size * 0.25:
                    # 选择交并比大于阈值的头30个候选坐标
                    if max_iou > 0.70:
                        # 交并比阈值0.7
                    ...
                    f_flag = 1
                if train_labels.count(0) < file_size * 0.75:
...
def data_loader(
    ...
    for i in tqdm(range(file_count)):
        ...
        if show:
            for j in range(len(ti_this)):
                print(tl_this[j])
                if tl_this[j] != 0 and tl_this[j] != 1:
                    image = ti_this[j]
    ...
    if balance:
        while train_labels.count(0) > 0.75 * len(train_labels):
